...
class Filter_Serach():
    ...
    def random_search_lambda(self, input, error, Lambda):
        ...
        for j, kernel_size in enumerate(self.Kernel_list):
            ...
            Conv1 = nn.Conv1d(self.In_channels,
                            self.Candidate_size, self.Kernel_size, padding=0).to(self.params.device)
            Conv1.weight.data = weight_candidate
            Conv1.bias.data = bias_candidates

            candidates_state = Conv1(input)
            # size(N, C_out, L_out)
            candidates_state = torch.sigmoid(candidates_state)
            candidates_state = self.Pool(candidates_state)

            for i,idx in enumerate(range(self.Candidate_size)):
                c_idx = candidates_state[:, idx, :]
                output_w = self.solve_output(c_idx, error)
                feature = torch.cat(
            (torch.ones(c_idx.size(0), 1).to(self.params.device), c_idx), 1)

                pred = feature.mm(output_w)
                loss = self.loss_fn(pred, error).item()
                if loss < min_loss:
                    min_loss = loss
                    self.conv_weight = weight_candidate[i,:,:].reshape(
                1, self.In_channels, kernel_size).detach().clone()
                    self.conv_bias = bias_candidates[i].view(
                -1).detach().clone()
    ...
    def random_search(self, input, error):
        with torch.no_grad():
            find = False
...
if __name__ == "__main__":
    ...
    params.model_name = 'escCNN_{}_h{}_{}'.format(
